...
#
# # START METADATA
#   recommendation = 91c18aee
#   function = ensure_telnet_client_not_installed
#   applicable =
# # END METADATA
#
#
...
ensure_telnet_client_not_installed()
{
	...
	ensure_telnet_client_not_installed_chk()
	{
		...
		# Check to see if telnet client is installed.  If not, we pass.
		if [ -z "$l_output" ]; then
			case "$G_PQ" in 
				*rpm*)
				...
				;;
				*dpkg*)
					if $G_PQ telnet || $G_PQ inetutils-telnet; then	
						# If telnet client is not installed, this command returns a "1", which means we go to the else clause and the test passes
						echo -e "- FAILED:\n- telnet package installed on the system" | tee -a "$LOG" 2>> "$ELOG"
						echo -e "- End check - Ensure telnet client is not installed" | tee -a "$LOG" 2>> "$ELOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
