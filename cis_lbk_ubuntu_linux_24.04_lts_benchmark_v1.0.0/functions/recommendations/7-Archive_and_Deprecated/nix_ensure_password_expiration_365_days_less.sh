...
#
# # START METADATA
#   recommendation = d8569535
#   function = ensure_password_expiration_365_days_less
#   applicable =
# # END METADATA
#
#
...
# David Neilson		11/24/23	   Changed "grep -E" to "grep -P --", and "\s" to "\h" in grep statements
   ...
   ensure_password_expiration_365_days_less_chk()
	{
      ...
      if grep -Pq -- '^\h*PASS_MAX_DAYS\h+([1-9]|[1-9][0-9]|[1-2][0-9]{2}|3[0-5][0-9]|36[0-5])\b' /etc/login.defs; then
         l_test1=passed
      elif grep -Pq -- '^\h*PASS_MAX_DAYS' /etc/login.defs; then
         l_output="- /etc/login.defs does NOT contain PASS_MAX_DAYS"
      else
         l_output="- /etc/login.defs PASS_MAX_DAYS set to: $(grep -Ps -- '^\h*PASS_MAX_DAYS' /etc/login.defs | awk '{print $2}')"
      fi
   ...
   ensure_password_expiration_365_days_less_fix()
	{
      ...
      if [ -z "$(awk -F : '/^root:[^!*]/ {print $5}' /etc/shadow)" ] || [ "$(awk -F : '/^root:[^!*]/ {print $5}' /etc/shadow)" -gt "365" ] || [ "$(awk -F : '/^root:[^!*]/ {print $5}' /etc/shadow)" = "-1" ]; then
         if chage --list root | grep -Pq -- '^Last password change\h*:\h*never' || [ "$(date --date="$(chage --list root | grep '^Last password change' | cut -d: -f2)" +%s)" -le "$((cde-30758400))" ]; then
            echo -e "- The password for root needs to be updated before setting expiration.  Please update root's password and set an expitation for root" | tee -a "$LOG" 2>> "$ELOG"
            l_test=manual
         fi
      fi
      ...
      if ! grep -Pq -- '^\h*PASS_MAX_DAYS\h+([1-9]|[1-9][0-9]|[1-2][0-9]{2}|3[0-5][0-9]|36[0-5])\b' /etc/login.defs; then
         if grep -Pq -- '^\h*PASS_MAX_DAYS\h+(-1|36[6-9]|3[7-9][0-9]|[4-9][0-9]{2}|[1-9][0-9]{3,})' /etc/login.defs; then
