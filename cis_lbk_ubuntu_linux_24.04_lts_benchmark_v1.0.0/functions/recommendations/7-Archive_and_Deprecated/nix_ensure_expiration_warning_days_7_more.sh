...
#
# # START METADATA
#   recommendation = 6f103753
#   function = ensure_expiration_warning_days_7_more
#   applicable =
# # END METADATA
#
#
...
# David Neilson		11/25/23	   Changed "grep -E" to "grep -P --", and "\s" to "\h" in grep statements
   ...
   ensure_expiration_warning_days_7_more_chk()
	{
      ...
      if grep -Pq -- '^\h*PASS_WARN_AGE\h+([7-9]|[1-9][0-9]+)\b' /etc/login.defs; then
         l_test1=passed
      elif [ -z "$(grep -P -- '^\h*PASS_WARN_AGE' /etc/login.defs)" ]; then
         l_output="- /etc/login.defs does NOT contain PASS_WARN_AGE"
      else
         l_output="- /etc/login.defs PASS_WARN_AGE set to: $(grep -Ps -- '^\h*PASS_WARN_AGE' /etc/login.defs | awk '{print $2}')"
      fi
   ...
   ensure_expiration_warning_days_7_more_fix()
	{
      ...
      if ! grep -Pq -- '^\h*PASS_WARN_AGE\h+([7-9]|[1-9][0-9]+)\b' /etc/login.defs; then
         if grep -Pq -- '^\h*PASS_WARN_AGE\h+\d+\b' /etc/login.defs; then
