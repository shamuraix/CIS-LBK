...
#
# # START METADATA
#   recommendation = 46adcb0d
#   function = ensure_avahi_server_not_installed
#   applicable =
# # END METADATA
#
#
...
# David Neilson		 10/26/23	 Changed "$G_PM -y remove" to "$G_PM remove -y"
# J Brown			 04/04/24	 This script will be deprecated and replaced by 'nix_ensure_avahi_daemon_services_not_in_use.sh'
#
			...
			case "$G_PQ" in
				*rpm*)
					if $G_PQ avahi-autopid | grep "not installed" && $G_PQ avahi | grep "not installed"; then
						...
						return "${XCCDF_RESULT_PASS:-101}"
					elif ! systemctl is-enabled avahi-daemon.socket avahi-daemon.service 2>/dev/null | grep 'enabled' && ! systemctl is-active avahi-daemon.socket avahi-daemon.service 2>/dev/null | grep '^active'; then
						echo -e "- PASSED:\n- avahi service is not running or enabled" | tee -a "$LOG" 2>> "$ELOG"
						echo -e "- End check - Ensure Avahi Server is not installed" | tee -a "$LOG" 2>> "$ELOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
