...
#
# # START METADATA
#   recommendation = dc1d4165
#   function = ensure_dev_shm_separate_partition
#   applicable =
# # END METADATA
#
#
...
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/25/22    Recommendation "Ensure /dev/shm is a separate partition"
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_dev_shm_separate_partition_fix()
   {
      ...
      if ! grep -Pq "^\h*[^#]+\h+$XCCDF_VALUE_REGEX(/)?\h+" /etc/fstab; then
         ...
         echo "tmpfs   /dev/shm    tmpfs   rw,noexec,nosuid,nodev,seclabel 0   0" >> /etc/fstab
      fi
