...
#
# # START METADATA
#   recommendation = d2840789
#   function = ensure_events_modify_user_group_information_collected
#   applicable =
# # END METADATA
#
#
...
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure events that modify user/group information are collected"
# David Neilson		 06/06/23	 Converted to generic format, and fixed various regex.  
# David Neilson		 11/03/23	 Does not check for, or remediate, /etc/gshadow on Suse system.
...
ensure_events_modify_user_group_information_collected()
{
	...
	ensure_events_modify_user_group_info_collected_chk()
	{
		echo "- Start check - Ensure events that modify user/group information are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

        # Set the files we're interested in.  The SLES benchmark does not include the /etc/gshadow file.
		if ! grep -Piq -- 'suse' /etc/os-release; then
			l_rule_1="/etc/group,/etc/passwd,/etc/gshadow,/etc/shadow,/etc/security/opasswd,/etc/nsswitch.conf,/etc/pam.conf,/etc/pam.d"
		else
			l_rule_1="/etc/group,/etc/passwd,/etc/shadow,/etc/security/opasswd,/etc/nsswitch.conf,/etc/pam.conf,/etc/pam.d"
		fi
		l_permissions_rule_1="w;a"

        # Verify ondisk rules with files
        echo -e "- Checking ondisk rules for $l_rule_1" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_watcher_rule "ondisk" "$l_rule_1" "$l_permissions_rule_1"

        # Verify auditctl entry with files
        echo -e "- Checking auditctl rules for $l_rule_1" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_watcher_rule "auditctl" "$l_rule_1" "$l_permissions_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure events that modify user/group information are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_events_modify_user_group_info_collected_fix()
	{
		echo "- Start remediation - Ensure events that modify user/group information are collected" | tee -a "$LOG" 2>> "$ELOG"
		
        # Fix ondisk rules with files
        echo -e "- Fixing ondisk rules for $l_rule_1" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_watcher_rule "$l_rule_1" "$l_permissions_rule_1" "identity" "/etc/audit/rules.d/50-identity.rules"

		echo "- End remediation - Ensure events that modify user/group information are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
