...
#
# # START METADATA
#   recommendation = 9cd3cb93
#   function = deb_ensure_use_privileged_commands_collected
#   applicable =
# # END METADATA
#
#
# CIS-LBK Recommendation Function
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_use_privileged_commands_collected.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure use of privileged commands are collected"
# 

deb_ensure_use_privileged_commands_collected()
{
	echo
	...
	deb_ensure_use_privileged_commands_collected_chk()
	{
		echo "- Start check - Ensure use of privileged commands are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

         echo -e "- NOTE: Due to the variable number of partitions, file systems and SUID/SGID executables that may exist on the system this item may take several minutes to run." | tee -a "$LOG" 2>> "$ELOG"

        # Collect the list of privileged commands to eval
        l_priv_cmds=""
        for l_partition in $(findmnt -n -l -k -it "$(awk '/nodev/ { print $2 }' /proc/filesystems | paste -sd,)" | grep -Pv "noexec|nosuid" | awk '{print $1}'); do
            for l_cmd in $(find "${l_partition}" -xdev -perm /6000 -type f); do
                if [ -z "$l_priv_cmds" ]; then
                    l_priv_cmds="$l_cmd"
                else
                    l_priv_cmds="$l_priv_cmds,$l_cmd"
                fi
            done
        done

        # Set the commands we're interested in
		l_rule_1="$l_priv_cmds"
        ...
        echo -e "- Checking ondisk rules for privileged commands" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_command_rule "noarch" "ondisk" "$l_rule_1" "$l_parameters_rule_1"

        # Verify auditctl entry with command
        echo -e "- Checking auditctl rules for privileged commands" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_command_rule "noarch" "auditctl" "$l_rule_1" "$l_parameters_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure use of privileged commands are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	deb_ensure_use_privileged_commands_collected_fix()
	{
		echo "- Start remediation - Ensure use of privileged commands are collected" | tee -a "$LOG" 2>> "$ELOG"
		
       # Fix ondisk rules with command
       echo -e "- Fixing ondisk rules privileged commands" | tee -a "$LOG" 2>> "$ELOG"
       fix_auditd_command_rule "noarch" "$l_rule_1" "$l_parameters_rule_1" "privileged" "/etc/audit/rules.d/50-privileged.rules"

		echo "- End remediation - Ensure use of privileged commands are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
