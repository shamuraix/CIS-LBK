...
#
# # START METADATA
#   recommendation = 3bc3e162
#   function = ensure_systemd-journal-remote_installed
#   applicable =
# # END METADATA
#
#
...
# Randie Bejar		 11/13/23    Updated to new version to only include rec IF journald will be used for logging
...
ensure_systemd-journal-remote_installed()
{
	...
	if pgrep rsyslogd >/dev/null 2>&1 && ! pgrep -x systemd-journald >/dev/null 2>&1; then
		...
		echo -e "- Recommendation Ensure systemd-journal-remote is installed is NA"  | tee -a "$LOG" 2>> "$ELOG"
		l_test="NA"
