...
#
# # START METADATA
#   recommendation = f7a23ce1
#   function = deb_ensure_kernel_module_loading_unloading_collected
#   applicable =
# # END METADATA
#
#
# CIS-LBK Recommendation Function
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_kernel_module_loading_unloading_collected.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure kernel module loading and unloading is collected"
# 

deb_ensure_kernel_module_loading_unloading_collected()
{
	echo
	...
	fix_auditd_command_rule(){
        ...
        for rule in "${rules_found_array[@]}"; do
            ...
            if (( ${#missing_parameter_array[@]} > 0 )); then
                ...
                rule_line_regex="$(grep -Po -- '^.+-F\s+path=\S+\s' <<< "$rule")"
                echo -e "-   Adding $parameter_insert_list" | tee -a "$LOG" 2>> "$ELOG"
                sed -ri "$rule_line s|($rule_line_regex)(.*)$|\1$parameter_insert_list \2|" "$rule_file"
            fi
        done
	...
	deb_ensure_kernel_module_loading_unloading_collected_chk()
	{
		echo "- Start check - Ensure kernel module loading and unloading is collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

		# check the rule(s) for create_module,init_module,delete_module,query_module,finit_module

		# Set the syscalls we're interested in
		l_rule_1="create_module,init_module,delete_module,query_module,finit_module"
		l_parameters_rule_1="-F auid>=$l_uid_min:-F auid>=$l_uid_min;-F auid!=unset:-F auid!=(unset|-1|auid!=4294967295)"

		# Verify b64 ondisk rules
		echo -e "- Checking arch=b64 ondisk rules for create_module,init_module,delete_module,query_module,finit_module" | tee -a "$LOG" 2>> "$ELOG"
		eval_auditd_syscall_rule "arch=b64" "ondisk" "$l_rule_1" "$l_parameters_rule_1"

		# Verify b64 auditctl entry
		echo -e "- Checking arch=b64 auditctl rules for create_module,init_module,delete_module,query_module,finit_module" | tee -a "$LOG" 2>> "$ELOG"
		eval_auditd_syscall_rule "arch=b64" "auditctl" "$l_rule_1" "$l_parameters_rule_1"

		# check the rule(s) for /usr/bin/kmod

		# Set the commands we're interested in
		l_rule_2="/usr/bin/kmod"
		l_parameters_rule_2="-F perm=x:-F perm=x;-F auid>=$l_uid_min:-F auid>=$l_uid_min;-F auid!=unset:-F auid!=(unset|-1|auid!=4294967295)"

		# Verify noarch ondisk rules
		echo -e "- Checking ondisk rules for /usr/bin/kmod" | tee -a "$LOG" 2>> "$ELOG"
		eval_auditd_command_rule "noarch" "ondisk" "$l_rule_2" "$l_parameters_rule_2"

		# Verify noarch auditctl entry
		echo -e "- Checking auditctl rules for /usr/bin/kmod" | tee -a "$LOG" 2>> "$ELOG"
		eval_auditd_command_rule "noarch" "auditctl" "$l_rule_2" "$l_parameters_rule_2"

		# check the symlinks to /bin/kmod
		echo -e "- Checking symlinks to /bin/kmod" | tee -a "$LOG" 2>> "$ELOG"
		l_kmod_links=""
		l_kmod_links="$(ls -l /usr/sbin/lsmod /usr/sbin/rmmod /usr/sbin/insmod /usr/sbin/modinfo /usr/sbin/modprobe /usr/sbin/depmod | grep -Pv -- " -> (..)?/bin/kmod" || true)"

		if [ -z "$l_kmod_links" ]; then
			l_output="$l_output\n- Symlinks to /bin/kmod all correctly configured"
		else
			l_output2="$l_output2\n- Issue with symlinks: ${l_kmod_links}\n"
		fi

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure kernel module loading and unloading is collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	deb_ensure_kernel_module_loading_unloading_collected_fix()
	{
		echo "- Start remediation - ensure kernel module loading and unloading is being collected" | tee -a "$LOG" 2>> "$ELOG"
		
		# Fix b64 rules with syscall init_module,finit_module,delete_module,create_module,query_module
		echo -e "- Fixing arch=b64 ondisk rules for init_module,finit_module,delete_module,create_module,query_module" | tee -a "$LOG" 2>> "$ELOG"
		fix_auditd_syscall_rule "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "kernel" "/etc/audit/rules.d/50-kernel_modules.rules"

		# Fix b64 rules with command /usr/bin/kmod
		echo -e "- Fixing noarch ondisk rules for /usr/bin/kmod" | tee -a "$LOG" 2>> "$ELOG"
		fix_auditd_command_rule "noarch" "$l_rule_2" "$l_parameters_rule_2" "kernel" "/etc/audit/rules.d/50-kernel_modules.rules"

		# Fix the symlinks to /bin/kmod
		
		l_kmod_links=""
		l_kmod_links="$(ls -l /usr/sbin/lsmod /usr/sbin/rmmod /usr/sbin/insmod /usr/sbin/modinfo /usr/sbin/modprobe /usr/sbin/depmod | grep -Pv -- " -> (..)?/bin/kmod" | awk -F"->" '{print $1}' | awk -F" " '{print $NF}' || true)"

		if [ -n "$l_kmod_links" ]; then
			echo -e "- Fixing symlinks to /bin/kmod" | tee -a "$LOG" 2>> "$ELOG"
			while lf_kmod_check= read -r lf_link; do
				echo -e "-  Linking $lf_link to /bin/kmod" | tee -a "$LOG" 2>> "$ELOG"
				ln -snf /bin/kmod "$lf_link"
			done <<< "$l_kmod_links"
		fi

		echo "- End remediation - ensure kernel module loading and unloading is being collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
