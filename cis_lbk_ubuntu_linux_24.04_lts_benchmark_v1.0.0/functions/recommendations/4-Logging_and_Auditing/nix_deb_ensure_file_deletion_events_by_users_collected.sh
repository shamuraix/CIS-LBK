...
#
# # START METADATA
#   recommendation = 2dd744c5
#   function = deb_ensure_file_deletion_events_by_users_collected
#   applicable =
# # END METADATA
#
#
# CIS-LBK Recommendation Function
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_file_deletion_events_by_users_collected.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure file deletion events by users are collected"
# 

deb_ensure_file_deletion_events_by_users_collected()
{
	echo
	...
	deb_ensure_file_deletion_events_by_users_collected_chk()
	{
		echo "- Start check - Ensure file deletion events by users are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""
		...
		l_rule_1="unlink,unlinkat,rename,renameat"
        ...
        if [ "$l_sysarch" = "b64" ]; then
            ...
            echo -e "- Checking arch=b64 ondisk rules for unlink,unlinkat,rename,renameat" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule "arch=b64" "ondisk" "$l_rule_1" "$l_parameters_rule_1"


            # Verify b64 auditctl entry
            echo -e "- Checking arch=b64 auditctl rules for unlink,unlinkat,rename,renameat" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule "arch=b64" "auditctl" "$l_rule_1" "$l_parameters_rule_1"
        fi

        # Verify b32 ondisk rules
        echo -e "- Checking arch=b32 ondisk rules for unlink,unlinkat,rename,renameat" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule "arch=b32" "ondisk" "$l_rule_1" "$l_parameters_rule_1"


        # Verify b32 auditctl entry
        echo -e "- Checking arch=b32 auditctl rules for unlink,unlinkat,rename,renameat" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule "arch=b32" "auditctl" "$l_rule_1" "$l_parameters_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure file deletion events by users are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	deb_ensure_file_deletion_events_by_users_collected_fix()
	{
		echo "- Start remediation - Ensure file deletion events by users are collected" | tee -a "$LOG" 2>> "$ELOG"
		
        if [ "$l_sysarch" = "b64" ]; then
            # Fix b64 ondisk rules
            echo -e "- Fixing arch=b64 ondisk rules for unlink,unlinkat,rename,renameat" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "delete" "/etc/audit/rules.d/50-delete.rules"
        fi

		# Fix b32 ondisk rules
        echo -e "- Fixing arch=b32 ondisk rules for unlink,unlinkat,rename,renameat" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_syscall_rule "arch=b32" "$l_rule_1" "$l_parameters_rule_1" "delete" "/etc/audit/rules.d/50-delete.rules"

		echo "- End remediation - Ensure file deletion events by users are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
