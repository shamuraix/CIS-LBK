...
#
# # START METADATA
#   recommendation = 8154fead
#   function = deb_ensure_events_modify_systems_mac_collected
#   applicable =
# # END METADATA
#
#
# CIS-LBK Recommendation Function
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_events_modify_systems_mac_collected.sh
# 
...
# David Neilson	     12/17/22	 Created for Ubuntu benchmarks
# J. Brown	         02/02/23	 Update to new format

deb_ensure_events_modify_systems_mac_collected()
{
	echo
	...
	deb_ensure_events_modify_systems_mac_collected_chk()
	{
		echo "- Start check - Ensure events that modify the system's Mandatory Access Controls are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""
				
		# Set the files and directories we're interested in
		l_rule_1="/etc/apparmor,/etc/apparmor.d"
		l_permissions_rule_1="w;a"

        # Verify ondisk rules with files and directories
        echo -e "- Checking ondisk rules for /etc/apparmor/,/etc/apparmor.d/" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_watcher_rule "ondisk" "$l_rule_1" "$l_permissions_rule_1"

        # Verify auditctl entry with files and directories
        echo -e "- Checking auditctl rules for /etc/apparmor/,/etc/apparmor.d/" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_watcher_rule "auditctl" "$l_rule_1" "$l_permissions_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure events that modify the system's Mandatory Access Controls are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	deb_ensure_events_modify_systems_mac_collected_fix()
	{
		echo "- Start remediation - Ensure events that modify the system's Mandatory Access Controls are collected" | tee -a "$LOG" 2>> "$ELOG"

		 # Fix ondisk rules with files and directories
        echo -e "- Fixing ondisk rules for /etc/apparmor/,/etc/apparmor.d/" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_watcher_rule "$l_rule_1" "$l_permissions_rule_1" "MAC-policy" "/etc/audit/rules.d/50-MAC-policy.rules"

		echo "- End remediation - Ensure events that modify the system's Mandatory Access Controls are collected" | tee -a "$LOG" 2>> "$ELOG"
	}
