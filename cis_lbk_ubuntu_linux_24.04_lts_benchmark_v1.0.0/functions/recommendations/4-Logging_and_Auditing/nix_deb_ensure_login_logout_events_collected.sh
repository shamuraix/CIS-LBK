...
#
# # START METADATA
#   recommendation = 748c69e7
#   function = deb_ensure_login_logout_events_collected
#   applicable =
# # END METADATA
#
#
...
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure login and logout events are collected"
# 
...
deb_ensure_login_logout_events_collected()
{
	...
	deb_ensure_login_logout_events_collected_chk()
	{
		echo "- Start check - Ensure login and logout events are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

        # Set the files and directories we're interested in
		l_rule_1="/var/log/lastlog,/var/run/faillock"
		l_permissions_rule_1="w;a"

        # Verify ondisk rules with files and directories
        echo -e "- Checking ondisk rules for /var/log/lastlog,/var/run/faillock" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_watcher_rule "ondisk" "$l_rule_1" "$l_permissions_rule_1"

        # Verify auditctl entry with files and directories
        echo -e "- Checking auditctl rules for /var/log/lastlog,/var/run/faillock" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_watcher_rule "auditctl" "$l_rule_1" "$l_permissions_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure login and logout events are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	deb_ensure_login_logout_events_collected_fix()
	{
		echo "- Start remediation - Ensure login and logout events are collected" | tee -a "$LOG" 2>> "$ELOG"
		...
		echo "- End remediation - Ensure login and logout events are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
