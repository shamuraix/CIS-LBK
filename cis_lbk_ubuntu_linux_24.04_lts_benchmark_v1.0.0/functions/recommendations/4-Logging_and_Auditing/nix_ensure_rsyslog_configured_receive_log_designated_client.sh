...
#
# # START METADATA
#   recommendation = d43431db
#   function = ensure_rsyslog_configured_receive_log_designated_client
#   applicable =
# # END METADATA
#
#
...
ensure_rsyslog_configured_receive_log_designated_client()
{
   ...
   ensure_rsyslog_configured_receive_log_designated_client_chk()
   {
      ...
      if grep -Pq '^\s*\$ModLoad imtcp' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
         l_output="$l_output\n- \$ModLoad imtcp was found in:\n$(grep -P '^\s*\$ModLoad imtcp' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)"
      else
         l_test1=passed
      ...
      if grep -Pq '^\s*\$InputTCPServerRun' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
         l_output="$l_output\n- \$InputTCPServerRun was found in:\n$(grep -P '^\s*\$InputTCPServerRun' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)"
      else
         l_test2=passed
      ...
      if grep -Pq '^\s*module\(load=\"imtcp\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
         l_output="$l_output\n- 'module(load=\"imtcp\")' was found in:\n$(grep -P '^\s*module\(load=\"imtcp\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)"
      else
         l_test3=passed
      ...
      if grep -Pq '^\s*input\(type=\"imtcp\" port=\"514\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
         l_output="$l_output\n- 'input(type=\"imtcp\" port=\"514\")' was found in:\n$(grep -P '^\s*input\(type=\"imtcp\" port=\"514\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)"
      else
         l_test4=passed
   ...
   ensure_rsyslog_configured_receive_log_designated_client_fix()
   {
      ...
      if [ "$l_test1" != "passed" ]; then
         if grep -Pq '\$ModLoad imtcp' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
            for l_file in $(grep '^\s*\$ModLoad imtcp' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null| awk -F: '{print $1}'); do
      ...
      if [ "$l_test2" != "passed" ]; then
         if grep -Pq '\$InputTCPServerRun' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
            for l_file in $(grep '^\s*\$InputTCPServerRun' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null| awk -F: '{print $1}'); do
      ...
      if [ "$l_test3" != "passed" ]; then
         if grep -Pq '^\s*module\(load=\"imtcp\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
            for l_file in $(grep -P '^\s*module\(load=\"imtcp\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null| awk -F: '{print $1}'); do
      ...
      if [ "$l_test4" != "passed" ]; then   
         if grep -Pq '^\s*input\(type=\"imtcp\" port=\"514\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 1>>/dev/null; then
            for l_file in $(grep -P '^\s*input\(type=\"imtcp\" port=\"514\"\)' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null| awk -F: '{print $1}'); do
