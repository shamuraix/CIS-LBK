...
#
# # START METADATA
#   recommendation = c0608755
#   function = ensure_audit_config_files_mode_640
#   applicable =
# # END METADATA
#
#
...
ensure_audit_config_files_mode_640()
{
	...
	ensure_audit_config_files_mode_640_chk()
	{
		...
		l_output=""
		l_output="$(find /etc/audit/ -type f \( -name '*.conf*' -o -name '*.rules*' \) -exec stat -Lc "%n %a" {} + | grep -Pv -- '^\h*\H+\h*([0,2,4,6][0,4]0|0)\h*$')"
