...
#
# # START METADATA
#   recommendation = 68893284
#   function = ensure_events_modify_sudo_log_file_collected
#   applicable =
# # END METADATA
#
#
...
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure events that modify the sudo log file are collected"
# David Neilson		 06/05/23	 Made generic, and updated various regex
# David Neilson      11/04/23    Changed name of variable "uid_min" in the Collect UID_MIN Value section to "l_uid_min".
...
ensure_events_modify_sudo_log_file_collected()
{
	...
	ensure_events_modify_sudo_log_collected_chk()
	{
		echo "- Start check - Ensure events that modify the sudo log file are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

        # Find the current sudo log or we default to /var/log/sudo.log
        l_sudo_log="$(grep -r -- logfile /etc/sudoers* | sed -e 's/.*logfile=//;s/,? .*//' -e 's/"//g')"

        # Set the commands we're interested in
		l_rule_1="${l_sudo_log:-/var/log/sudo.log}"
		...
		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure events that modify the sudo log file are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_events_modify_sudo_log_collected_fix()
	{
        ...
        fix_auditd_watcher_rule "$l_rule_1" "$l_permissions_rule_1" "sudo_log_file" "/etc/audit/rules.d/50-sudo.rules"

		echo "- End remediation - Ensure events that modify the sudo log file are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
