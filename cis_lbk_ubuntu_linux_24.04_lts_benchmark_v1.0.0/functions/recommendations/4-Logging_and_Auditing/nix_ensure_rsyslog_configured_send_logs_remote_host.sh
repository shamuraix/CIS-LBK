...
#
# # START METADATA
#   recommendation = 497cc957
#   function = ensure_rsyslog_configured_send_logs_remote_host
#   applicable =
# # END METADATA
#
#
...
ensure_rsyslog_configured_send_logs_remote_host()
{
   ...
   ensure_rsyslog_configured_send_logs_remote_host_chk()
   {
      ...
      if grep -E '^\s*([^#]+\s+)?action\(([^#]+\s+)?\btarget=\"?[^#"]+\"?\b' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null; then
         	l_output="Remote log host entry found:\n$(grep -E '^\s*([^#]+\s+)?action\(([^#]+\s+)?\btarget=\"?[^#"]+\"?\b' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)"
         	l_test="passed"
      fi

      if grep -E '^[^#]\s*\S+\s+\@' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null; then
         	l_output="$l_output\nRemote log host entry found:\n$(grep -E '^[^#]\s*\S+\s+\@' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)"
         	l_test="passed"
      fi         
