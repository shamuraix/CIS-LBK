...
#
# # START METADATA
#   recommendation = 03392d50
#   function = ensure_rsyslog_default_file_permissions_configured
#   applicable =
# # END METADATA
#
#
...
# David Neilson		11/06/23	   Changed references of "$FileCreateMode 0640" to "\$FileCreateMode 0640"
...
ensure_rsyslog_default_file_permissions_configured()
{
   ...
   ensure_rsyslog_default_file_permissions_configured_chk()
   {
      ...
      if grep -Eq '^\s*\$[Ff]ile[Cc]reate[Mm]ode\s+0[6420][04]0\b' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null; then
         echo -e "- PASS:\n- \$FileCreateMode entry found: $(grep -E '^\s*\$[Ff]ile[Cc]reate[Mm]ode\s+0[6420][04]0\b' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure rsyslog default file permissions configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
      else
         if grep -Eq '^\s*(#)?\s*\$[Ff]ile[Cc]reate[Mm]ode' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null; then
            echo -e "- FAIL:\n- \$FileCreateMode entry found:\n- $(grep -E '^\s*(#)?\s*\$[Ff]ile[Cc]reate[Mm]ode' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null)" | tee -a "$LOG" 2>> "$ELOG"
            echo -e "- End check - Ensure rsyslog default file permissions configured" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_rsyslog_default_file_permissions_configured_fix()
   {
      ...
      if grep -Eq '^\s*(#)?\s*\$[Ff]ile[Cc]reate[Mm]ode' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null; then
         for l_file in $(grep -E '^\s*(#)?\s*\$[Ff]ile[Cc]reate[Mm]ode' /etc/rsyslog.conf /etc/rsyslog.d/*.conf 2>/dev/null| awk -F: '{print $1}' 2>/dev/null); do
