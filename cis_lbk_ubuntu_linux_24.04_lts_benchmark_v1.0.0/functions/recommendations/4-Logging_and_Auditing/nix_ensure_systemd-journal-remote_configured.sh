...
#
# # START METADATA
#   recommendation = fa656d86
#   function = ensure_systemd-journal-remote_configured
#   applicable =
# # END METADATA
#
#
...
ensure_systemd-journal-remote_configured()
{
   ...
   ensure_systemd-journal-remote_configured_chk()
   {
        ...
        echo -e "- Currently configured settings are:\n$(grep -P "^\s*URL=|^\s*ServerKeyFile=|^\s*ServerCertificateFile=|^\s*TrustedCertificateFile=" /etc/systemd/journal-upload.conf 2>/dev/null)" | tee -a "$LOG" 2>> "$ELOG"
