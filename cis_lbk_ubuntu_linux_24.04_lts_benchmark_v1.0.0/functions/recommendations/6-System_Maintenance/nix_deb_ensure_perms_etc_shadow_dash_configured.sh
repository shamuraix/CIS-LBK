...
#   function = deb_ensure_perms_etc_shadow_dash_configured
#   applicable =
# # END METADATA
#
#
# CIS-LBK Cloud Team Built Recommendation Function
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_perms_etc_shadow_dash_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       11/09/22     Recommendation "Ensure permissions on /etc/shadow- are configured"

deb_ensure_perms_etc_shadow_dash_configured()
{
	...
	deb_ensure_perms_etc_shadow_dash_configured_chk()
	{
		# Checks for correctly set permissions and ownership
		echo "- Start check - Ensure permissions on /etc/shadow- are configured" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""
		...
		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output"  | tee -a "$LOG" 2>> "$ELOG"
		   	echo -e "- End check - Ensure permissions on /etc/shadow- are configured" | tee -a "$LOG" 2>> "$ELOG"
		   	return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
		   	echo -e "- FAILED:\n- Incorrect values:\n$l_output2"  | tee -a "$LOG" 2>> "$ELOG"
			[ -n "$l_output" ] && echo -e "\nCorrect values:\n$l_output" | tee -a "$LOG" 2>> "$ELOG"
		   	echo "- End check - Ensure permissions on /etc/shadow- are configured" | tee -a "$LOG" 2>> "$ELOG"
		   	return "${XCCDF_RESULT_FAIL:-102}"
		fi	
	}

	deb_ensure_perms_etc_shadow_dash_configured_fix()
	{
        echo -e "- Start remediation - Ensure permissions on /etc/shadow- are configured" | tee -a "$LOG" 2>> "$ELOG"
        ...
        echo -e "- End remediation - Ensure permissions on /etc/shadow- are configured" | tee -a "$LOG" 2>> "$ELOG"
	}
