...
#
# # START METADATA
#   recommendation = ad8a71df
#   function = ensure_perms_etc_shells_configured
#   applicable =
# # END METADATA
#
#
# CIS-LBK Remediation Function
# ~/CIS-LBK/functions/recommendations/nix_ensure_perms_etc_shells_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J Brown             07/02/23     Recommendation "Ensure permissions on /etc/shells are configured"

ensure_perms_etc_shells_configured()
{
	...
	ensure_perms_etc_shells_configured_chk()
	{
		# Checks for correctly set permissions and ownership
		echo "- Start check - Ensure permissions on /etc/shells are configured" | tee -a "$LOG" 2>> "$ELOG"
         l_output="" l_output2=""
		
        for file in /etc/shells; do
            if [ -e $file ]; then
                ...
                if ! grep -Pq '[0-6][0-4][0-4]' <<< "$l_perms"; then
        ...
        if [ -z "$l_output2" ]; then
            ...
            echo "- End check - Ensure permissions on /etc/shells are configured" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_perms_etc_shells_configured_fix()
	{
		echo "- Start remediation - Ensure permissions on /etc/shells are configured" | tee -a "$LOG" 2>> "$ELOG"
        ...
        echo "- End remediation - Ensure permissions on /etc/shells are configured" | tee -a "$LOG" 2>> "$ELOG"
	}
