...
#   function = deb_ensure_pam_unix_does_not_include_remember
#   applicable =
# # END METADATA
#
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_pam_unix_does_not_include_remember.sh
#
...
deb_ensure_pam_unix_does_not_include_remember()
{
	echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
	l_test=""

	deb_ensure_pam_unix_does_not_include_remember_chk()
	{
		echo -e "- Start check - Ensure pam_unix does not include remember" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

		# Verify common-account
		if grep -Pqs -- '^\h*account\s+([^#\r\n]+)?\h+pam_unix\.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-account; then
			l_output2="$l_output2\n- /etc/pam.d/common-account contains:\n  $(grep -Ps -- '^\h*account\s+([^#\r\n]+)?\h+pam_unix\.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-account)"
		else
			l_output="$l_output\n- /etc/pam.d/common-account DOES NOT include 'remember'"
		fi

		# Verify common-session
		if grep -Pqs -- '^\h*session\h+([^#\r\n]+)?\h+pam_unix.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-session; then
			l_output2="$l_output2\n- /etc/pam.d/common-session contains:\n  $(grep -Ps -- '^\h*session\h+([^#\r\n]+)?\h+pam_unix.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-session)"
		else
			l_output="$l_output\n- /etc/pam.d/common-session DOES NOT include 'remember'"
		fi

		# Verify common-auth
		if grep -Pqs -- '^\h*auth\h+([^#\r\n]+)?\h+pam_unix.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-auth; then
			l_output2="$l_output2\n- /etc/pam.d/common-auth contains:\n  $(grep -Ps -- '^\h*auth\h+([^#\r\n]+)?\h+pam_unix.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-auth)"
		else
			l_output="$l_output\n- /etc/pam.d/common-auth DOES NOT include 'remember'"
		fi

		# Verify common-password
		if grep -Pqs -- '^\h*password\h+([^#\r\n]+)?\h+pam_unix.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-password; then
			l_output2="$l_output2\n- /etc/pam.d/common-password contains:\n  $(grep -Ps -- '^\h*password\h+([^#\r\n]+)?\h+pam_unix.so([^#\r\n]+)?\bremember=\d+\b' /etc/pam.d/common-password)"
		else
			l_output="$l_output\n- /etc/pam.d/common-password DOES NOT include 'remember'"
		fi
	...
	deb_ensure_pam_unix_does_not_include_remember_fix()
	{
        ...
        l_config_files="$(grep -Pl -- '\bpam_unix\.so\h+([^#\n\r]+\h+)?\bremember=\d+\b' /usr/share/pam-configs/*)"
        if [ -n "$l_config_files" ]; then
            while read -r l_file; do
                echo -e "- Removing 'remember' values from $l_file" | tee -a "$LOG" 2>> "$ELOG"
                sed -ri 's/^(.*pam_unix\.so)([^#]+\s*)?(remember=[0-9]+)(.*)?$/\1\2\4/g' "$l_file"
            done <<< "$l_config_files"
		...
		echo -e "- End remediation - Ensure pam_unix does not include remember" | tee -a "$LOG" 2>> "$ELOG"
	}
