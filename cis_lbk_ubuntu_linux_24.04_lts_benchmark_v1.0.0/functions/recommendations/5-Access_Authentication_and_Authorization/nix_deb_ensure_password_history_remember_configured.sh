...
#   function = deb_ensure_password_history_remember_configured
#   applicable =
# # END METADATA
#
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_password_history_remember_configured.sh
#
# Name                  Date            Description
# ------------------------------------------------------------------------------------------------
# J Brown               03/28/24        Recommendation "Ensure password history remember is configured"
#

deb_ensure_password_history_remember_configured()
{
	echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
    l_test=""

    deb_ensure_password_history_remember_configured_chk()
	{
        ...
        l_pwhistory_config="$(grep -Psi -- '^\h*password\h+[^#\n\r]+\h+pam_pwhistory\.so\h+([^#\n\r]+\h+)?remember=([0-9]|1[0-9]|2[0-3])\b' /etc/pam.d/common-password)"

        if [ -z "$l_pwhistory_config" ]; then
            l_output="$l_output\n- No non-compliant 'remember' values configured in /etc/pam.d/common-password"
        else
            l_output2="$l_output2\n- 'remember' value(s) NOT configured correctly in /etc/pam.d/common-password:\n$l_pwhistory_config"
        fi
    ...
    deb_ensure_password_history_remember_configured_fix()
	{
        ...
        l_config_files="$(grep -Pl -- '\bpam_pwhistory\.so\h+([^#\n\r]+\h+)?remember=([0-9]|1[0-9]|2[0-3])\b' /usr/share/pam-configs/*)"

        if [ -n "$l_config_files" ]; then
            while read -r l_file; do
                echo -e "- Updating 'remember' values in $l_file" | tee -a "$LOG" 2>> "$ELOG"
                sed -ri 's/^(.*pam_pwhistory\.so)([^#]+\s*)?(remember\s*=\s*)([0-9]+)(.*)?$/\1\2\324\5/' "$l_file"
            done <<< "$l_config_files"
