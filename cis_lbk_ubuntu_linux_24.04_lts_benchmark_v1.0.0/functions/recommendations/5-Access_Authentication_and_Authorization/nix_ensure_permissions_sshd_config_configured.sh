...
#   function = ensure_permissions_sshd_config_configured
#   applicable =
# # END METADATA
#
#
...
# David Neilson		 11/13/23	 Changed "$G_PM -y remove" to "$G_PM remove -y", changed variable "test" to "l_test", and "tst1" and "tst2" to "l_tst1" and "l_tst2"
...
ensure_permissions_sshd_config_configured()
{
	...
	{
		output="" l_fperm="" l_fog="" l_tst1="" l_tst2=""
		...
		if [ -f "$sshd_config_file" ]; then
			...
			grep -Pq -- '^\h*[0,2,4,6]00\h*$' <<< "$l_fperm" && l_tst1="pass"
			grep -Pq -- '^\h*root\h+root\h*$' <<< "$l_fog" && l_tst2="pass"
