...
#
# # START METADATA
#   recommendation = 0563e68d
#   function = ensure_permissions_etc_cron_daily_configured
#   applicable =
# # END METADATA
#
#
# CIS-LBK _Main Recommendation Function
# ~/CIS-LBK/functions/recommendations/nix_ensure_permissions_etc_cron_daily_configured.sh
#
# Name                  Date            Description
# --------------------------------------------------------------------------------------------------------
# J Brown               02/21/24        Recommendation "Ensure permissions on /etc/cron.daily are configured"
# David Neilson         06/28/24        Set l_test variable instead of running the "return" command if package manager is unknown or cron not installed, changed references of "0600" to "0700", and changed variable "cron_dir" to "l_cron_dir".


ensure_permissions_etc_cron_daily_configured()
{
	echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
    l_test="" l_pkg=""
    l_cron_dir="/etc/cron.daily/"
	...
	ensure_permissions_etc_cron_daily_configured_chk()
	{
	...
	ensure_permissions_etc_cron_daily_configured_fix()
	{
