...
#
# # START METADATA
#   recommendation = a0c05ef9
#   function = deb_ensure_nftables_table_exists
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_nftables_table_exists.sh
# 
...
deb_ensure_nftables_table_exists()
{
	...
	deb_ensure_nftables_table_exists_chk()
	{
      ...
      l_nftables=$(nft list tables)
		...
		if [ -n "$l_nftables" ] ; then
			echo -e "- An nftables table exists:\n$l_nftables" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure an nftables table exists" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_nftables_table_exists_fix()
	{
