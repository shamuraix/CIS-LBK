...
#
# # START METADATA
#   recommendation = 85e27caa
#   function = ensure_tipc_disabled
#   applicable =
# # END METADATA
#
#
...
ensure_tipc_disabled()
{
   ...
   {
      echo -e "- Start check - Ensure TIPC is disabled" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ -z "$l_output2" ]; then
         echo -e "\n- Audit Result:\n  ** PASS **\n$l_output\n"
         echo -e "- End check - Ensure TIPC is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   {
      echo -e "- Start remediation - Ensure TIPC is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   echo -e "\n - remediation of module: \"$l_mname\" complete\n"
      ...
      echo -e "- End remediation - Ensure TIPC is disabled" | tee -a "$LOG" 2>> "$ELOG"
   }
