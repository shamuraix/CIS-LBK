...
#
# # START METADATA
#   recommendation = 1a9e5bf8
#   function = deb_ensure_iptables_packages_installed
#   applicable =
# # END METADATA
#
...
# ------------------------------------------------------------------------------------------------
# Justin Brown       06/28/22    Recommendation "Ensure iptables packages are installed"
# Gokhan Lus		 04/25/24	 Added lines 297 & 298 to install iptables-persistent properly on Debian 11
...
deb_ensure_iptables_packages_installed()
{
	...
	deb_ensure_iptables_packages_installed_chk()
	{
		echo "- Start check - Ensure iptables packages are installed" | tee -a "$LOG" 2>> "$ELOG"
		output=""
		...
		! $G_PQ iptables >/dev/null 2>&1 && output="- The IPtables packages are not installed on the system"
      	! $G_PQ iptables-persistent >/dev/null 2>&1 && output="- The IPtables-Persistent packages are not installed on the system"

		# If package doesn't exist, we pass
		if [ -z "$output" ] ; then
			echo "- The iptables packages are installed" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure iptables packages are installed" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_iptables_packages_installed_fix()
	{
		echo -e "- Start remediation - Ensure iptables packages are installed" | tee -a "$LOG" 2>> "$ELOG"
		$G_PM install -y iptables
		echo iptables-persistent iptables-persistent/autosave_v4 boolean true | sudo debconf-set-selections
		echo iptables-persistent iptables-persistent/autosave_v6 boolean true | sudo debconf-set-selections
      	$G_PM install -y iptables-persistent
		echo -e "- End remediation - Ensure iptables packages are installed" | tee -a "$LOG" 2>> "$ELOG"
	}
