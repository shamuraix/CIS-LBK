...
#
# # START METADATA
#   recommendation = 186aa2e7
#   function = deb_ensure_iptables_flushed_with_nftables
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_iptables_flushed_with_nftables.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       06/24/22    Recommendation "Ensure iptables are flushed with nftables"
# Justin Brown		 09/08/22 	 Small syntax changes
#

deb_ensure_iptables_flushed_with_nftables()
{
	...
	deb_firewall_chk()
	{
		...
		# Check for multiple firewalls
		elif [[ "$l_fwds" =~ yy. && "$l_nfts" =~ yy. ]] || [[ "$l_fwds" =~ yy. && "$l_ipts" =~ yy. ]] || [[ "$l_fwds" =~ yy. && "$l_ufws" =~ yy. ]] || [[ "$l_nfts" =~ yy. && "$l_ipts" =~ yy. ]] || [[ "$l_nfts" =~ yy. && "$l_ufws" =~ yy. ]] || [[ "$l_ipts" =~ yy. && "$l_ufws" =~ yy. ]]; then
			G_FWIN="MFu"
		else
			...
			# Check for NFTables
			if [[ "$l_nfts" =~ yy. && "$l_fwds" =~ .nn && "$l_ufws" =~ .nn && "$l_ipts" =~ .nn ]]; then
				G_FWIN="NFt"
			fi
			# Check for IPTables
	...
	deb_ensure_iptables_flushed_with_nftables_chk()
	{
		echo "- Start check - Ensure iptables are flushed with nftables" | tee -a "$LOG" 2>> "$ELOG"
      ...
      l_iptables_rules=$(iptables -L | grep -Pv '^(Chain|target)')
      
      # Collect ip6tables rules
      l_ip6tables_rules=$(ip6tables -L | grep -Pv '^(Chain|target)')
		
		# If iptables rules don't exist, we pass
		if [ -z "$l_iptables_rules" ] && [ -z "$l_ip6tables_rules" ] ; then
			echo "- No iptables rules exist" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure iptables are flushed with nftables" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
         if [ -n "$l_iptables_rules" ]; then
            echo -e "- iptables rules found:\n$l_iptables_rules" | tee -a "$LOG" 2>> "$ELOG"
         fi
         if [ -n "$l_ip6tables_rules" ]; then
            echo -e "- ip6tables rules found:\n$l_ip6tables_rules" | tee -a "$LOG" 2>> "$ELOG"
         fi
			echo "- End check - Ensure iptables are flushed with nftables" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	...
	deb_ensure_iptables_flushed_with_nftables_fix()
	{
		echo -e "- Start remediation - Ensure iptables are flushed with nftables" | tee -a "$LOG" 2>> "$ELOG"
		
      echo -e "- Flushing iptables rules" | tee -a "$LOG" 2>> "$ELOG"
      iptables -F && test=remediated
      
      echo -e "- Flushing ip6tables rules" | tee -a "$LOG" 2>> "$ELOG"
      ip6tables -F && test=remediated
      
		echo -e "- End remediation - Ensure iptables are flushed with nftables" | tee -a "$LOG" 2>> "$ELOG"
	}
