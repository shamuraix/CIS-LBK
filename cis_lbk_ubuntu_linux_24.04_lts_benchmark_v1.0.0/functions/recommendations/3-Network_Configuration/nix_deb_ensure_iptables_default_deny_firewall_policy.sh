...
#
# # START METADATA
#   recommendation = bd62f82b
#   function = deb_ensure_iptables_default_deny_firewall_policy
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_iptables_default_deny_firewall_policy.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/02/22    Recommendation "Ensure iptables default deny firewall policy"
#

deb_ensure_iptables_default_deny_firewall_policy()
{
	...
	deb_ensure_iptables_default_deny_firewall_policy_chk()
	{
		echo "- Start check - Ensure iptables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
		l_input_default_deny="" l_forward_default_deny="" l_output_default_deny="" l_ruleset=""
		            
		# Collect nftables tables
      l_input_default_deny=$(iptables -L | grep -Pi '^\s*chain\s+input\s+' | grep -Piv 'policy\s+drop')
      l_forward_default_deny=$(iptables -L | grep -Pi '^\s*chain\s+forward\s+' | grep -Piv 'policy\s+drop')
      l_output_default_deny=$(iptables -L | grep -Pi '^\s*chain\s+output\s+' | grep -Piv 'policy\s+drop')
      ...
      # if ruleset passes, we pass
		if [ "$l_ruleset" = passed ]; then
			...
			echo -e "- End check - Ensure iptables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_iptables_default_deny_firewall_policy_fix()
	{
		echo -e "- Start remediation - Ensure iptables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"

      echo -e "- Update the chains to enforce a default policy of drop." | tee -a "$LOG" 2>> "$ELOG"
      echo -e "- These example rules can be used if desired:\n iptables -P INPUT DROP\n iptables -P OUTPUT DROP\n iptables -P FORWARD DROP" | tee -a "$LOG" 2>> "$ELOG"
      test=manual
      
		echo -e "- End remediation - Ensure iptables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
	}
