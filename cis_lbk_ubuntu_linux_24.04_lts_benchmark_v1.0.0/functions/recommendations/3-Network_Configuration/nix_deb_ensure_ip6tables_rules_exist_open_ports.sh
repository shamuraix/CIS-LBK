...
#
# # START METADATA
#   recommendation = 65869c78
#   function = deb_ensure_ip6tables_rules_exist_open_ports
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ip6tables_rules_exist_open_ports.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/02/22    Recommendation "Ensure ip6tables firewall rules exist for all open ports"
#

deb_ensure_ip6tables_rules_exist_open_ports()
{
	...
	deb_ensure_ip6tables_rules_exist_open_ports_chk()
	{
		echo "- Start check - Ensure ip6tables firewall rules exist for all open ports" | tee -a "$LOG" 2>> "$ELOG"
		
      # Collect open ports
      l_open_ports="$(ss -6Hln | grep -v '::1' | awk '{ split($5,a,":"); print $1 " " a[4]}')"
      ...
      # if ruleset passes, we pass
		if [ "$l_rules_test" = passed ]; then
			...
			echo -e "- End check - Ensure ip6tables firewall rules exist for all open ports" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
         echo -e "- Open ports found:\n$l_open_ports\n" | tee -a "$LOG" 2>> "$ELOG"
         echo -e "- Current ip6tables rules:\n$(ip6tables -L INPUT -v -n)\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure ip6tables firewall rules exist for all open ports" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_ip6tables_rules_exist_open_ports_fix()
	{
		echo -e "- Start remediation - Ensure ip6tables firewall rules exist for all open ports" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- Some open ports exist. Review the list of open ports and configure the appropriate rules based on the organizational policy." | tee -a "$LOG" 2>> "$ELOG" && test=manual
		...
		echo -e "- End remediation - Ensure ip6tables firewall rules exist for all open ports" | tee -a "$LOG" 2>> "$ELOG"
	}
