...
#
# # START METADATA
#   recommendation = 13b70f04
#   function = ensure_sctp_disabled
#   applicable =
# # END METADATA
#
#
...
ensure_sctp_disabled()
{
   ...
   {
      echo -e "- Start check - Ensure SCTP is disabled" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ -z "$l_output2" ]; then
         echo -e "\n- Audit Result:\n  ** PASS **\n$l_output\n"
         echo -e "- End check - Ensure SCTP is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   {
      echo -e "- Start remediation - Ensure SCTP is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   echo -e "\n - remediation of module: \"$l_mname\" complete\n"
      ...
      echo -e "- End remediation - Ensure SCTP is disabled" | tee -a "$LOG" 2>> "$ELOG"
   }
