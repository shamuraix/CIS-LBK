...
#
# # START METADATA
#   recommendation = 2ca3fefc
#   function = ensure_rds_disabled
#   applicable =
# # END METADATA
#
#
...
# ------------------------------------------------------------------------------------------------
# Justin Brown       11/19/22    Recommendation "Ensure RDS is disabled"
...
ensure_rds_disabled()
{
   ...
   ensure_rds_disabled_chk()
   {
      echo -e "- Start check - Ensure RDS is disabled" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ -z "$l_output2" ]; then
         echo -e "\n- Audit Result:\n  ** PASS **\n$l_output\n"
         echo -e "- End check - Ensure RDS is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_rds_disabled_fix()
   {
      echo -e "- Start remediation - Ensure RDS is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   echo -e "\n - remediation of module: \"$l_mname\" complete\n"
      ...
      echo -e "- End remediation - Ensure RDS is disabled" | tee -a "$LOG" 2>> "$ELOG"
   }
