...
#
# # START METADATA
#   recommendation = 17a7dccf
#   function = deb_ensure_ufw_not_installed_or_disabled_with_nftables
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ufw_not_installed_or_disabled_with_nftables.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       06/24/22    Recommendation "Ensure ufw is uninstalled or disabled with nftables"
#

deb_ensure_ufw_not_installed_or_disabled_with_nftables()
{
	...
	deb_ensure_ufw_not_installed_or_disabled_with_nftables_chk()
	{
		echo "- Start check - Ensure ufw is uninstalled or disabled with nftables" | tee -a "$LOG" 2>> "$ELOG"
		output=""
		...
		if [ -z "$output" ] ; then
			echo "- End check - Ensure ufw is uninstalled or disabled with nftables" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_ufw_not_installed_or_disabled_with_nftables_fix()
	{
		echo -e "- Start remediation - Ensure ufw is uninstalled or disabled with nftables" | tee -a "$LOG" 2>> "$ELOG"

		echo -e "- Disabling ufw service" | tee -a "$LOG" 2>> "$ELOG"
		ufw disable
		systemctl disable ufw

		echo -e "- End remediation - Ensure ufw is uninstalled or disabled with nftables" | tee -a "$LOG" 2>> "$ELOG"
	}
