...
#
# # START METADATA
#   recommendation = 3ffaea41
#   function = deb_ensure_nftables_not_installed_with_iptables_services
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_nftables_not_installed_with_iptables_services.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       06/24/22    Recommendation "Ensure nftables is not installed with iptables"
#

deb_ensure_nftables_not_installed_with_iptables_services()
{
	...
	deb_ensure_nftables_not_installed_with_iptables_services_chk()
	{
		echo "- Start check - Ensure nftables is not installed with iptables" | tee -a "$LOG" 2>> "$ELOG"
		output=""
		...
		$G_PQ nftables >/dev/null 2>&1 && output="- The nftables package is installed on the system"
		
		# If package doesn't exist, we pass
		if [ -z "$output" ] ; then
			echo "- The nftables package is not installed" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure nftables is not installed with iptables" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_nftables_not_installed_with_iptables_services_fix()
	{
		echo -e "- Start remediation - Ensure nftables is not installed with iptables" | tee -a "$LOG" 2>> "$ELOG"
		$G_PR nftables
		echo -e "- End remediation - Ensure nftables is not installed with iptables" | tee -a "$LOG" 2>> "$ELOG"
	}
