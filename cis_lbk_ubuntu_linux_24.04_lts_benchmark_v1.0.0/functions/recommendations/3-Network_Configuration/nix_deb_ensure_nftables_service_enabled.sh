...
#
# # START METADATA
#   recommendation = 66267d68
#   function = deb_ensure_nftables_service_enabled
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_nftables_service_enabled.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       05/18/22    Recommendation "Ensure nftables service is enabled"
#

deb_ensure_nftables_service_enabled()
{
	...
	deb_ensure_nftables_service_enabled_chk()
	{
		echo "- Start check - Ensure nftables service is enabled" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_nfts=""
		...
		if $G_PQ nftables >/dev/null; then
			l_nfts="$(systemctl is-enabled nftables)"
			! grep -q 'enabled' <<< "$l_nfts" && l_output="$l_output\n- nftables service is: $l_nfts"
		else
		...
		if [ -z "$l_output" ]; then
			echo -e "- The nftables service is: $l_nfts" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure nftables service is enabled" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_nftables_service_enabled_fix()
	{
		echo -e "- Start remediation - Ensure nftables service is enabled" | tee -a "$LOG" 2>> "$ELOG"
		echo -e "- Enabling and starting the nftables service" | tee -a "$LOG" 2>> "$ELOG"
		systemctl unmask nftables
		systemctl --now start nftables
		echo -e "- End remediation - Ensure nftables service is enabled" | tee -a "$LOG" 2>> "$ELOG"
	}
