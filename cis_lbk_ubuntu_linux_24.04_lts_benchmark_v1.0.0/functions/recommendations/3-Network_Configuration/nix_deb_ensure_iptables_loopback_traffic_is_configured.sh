...
#
# # START METADATA
#   recommendation = b6ff803d
#   function = deb_ensure_iptables_loopback_traffic_is_configured
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_iptables_loopback_traffic_is_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       06/28/22    Recommendation "Ensure iptables loopback traffic is configured"
#

deb_ensure_iptables_loopback_traffic_is_configured()
{
	...
	deb_ensure_iptables_loopback_traffic_is_configured_chk()
	{
		echo "- Start check - Ensure iptables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
		output="" l_input_chain="" l_output_chain=""
		            
		# Get INPUT Chain rules
      l_in_lo_rule="$(iptables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+--\s+lo\s+\*')"
      l_in_127_rule="$(iptables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+DROP\s+all\s+--\s+\*\s+\*\s+127\.0\.0\.0\/8')"
      
      # Get OUTPUT Chain rules
      l_out_lo_rule="$(iptables -L OUTPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+--\s+\*\s+lo')"
      ...
      if [ -n "$l_in_lo_rule" ]; then
		...
		if [ "$l_ruleset" = passed ] ; then
			echo "- The iptables rules for loopback are configured" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure iptables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo "- The iptables rules for loopback are NOT configured" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure iptables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_iptables_loopback_traffic_is_configured_fix()
	{
		echo -e "- Start remediation - Ensure iptables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"

      if [ -z "$(iptables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+--\s+lo\s+\*')" ]; then
         echo "- Adding iptables INPUT rules for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
         iptables -A INPUT -i lo -j ACCEPT && test=remediated
      fi
      
      if [ -z "$(iptables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+DROP\s+all\s+--\s+\*\s+\*\s+127\.0\.0\.0\/8')" ]; then
         echo "- Adding iptables INPUT rules for loopback network" | tee -a "$LOG" 2>> "$ELOG"
         iptables -A INPUT -s 127.0.0.0/8 -j DROP && test=remediated
      fi
      
      if [ -z "$(iptables -L OUTPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+--\s+\*\s+lo')" ]; then
         echo "- Adding iptables OUTPUT rules for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
         iptables -A OUTPUT -o lo -j ACCEPT && test=remediated
      fi
      
		echo -e "- End remediation - Ensure iptables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
	}
