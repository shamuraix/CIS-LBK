...
#
# # START METADATA
#   recommendation = 459acead
#   function = deb_ensure_ip6tables_outbound_established_connections_configured
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ip6tables_outbound_established_connections_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       06/28/22    Recommendation "Ensure ip6tables outbound and established connections are configured"
#

deb_ensure_ip6tables_outbound_established_connections_configured()
{
   ...
   ipv6_chk()
   {
	...
	deb_ensure_ip6tables_outbound_established_connections_configured_chk()
	{
		echo "- Start check - Ensure ip6tables outbound and established connections are configured" | tee -a "$LOG" 2>> "$ELOG"
      ...
      l_input_tcp_established=$(ip6tables -L -v -n | grep -Pi '^\s*\d+\s+\d+\s+ACCEPT\s+tcp\s+\*\s+\*\s+::\/0\s+::\/0\s+state\s+established')
      l_input_udp_established=$(ip6tables -L -v -n | grep -Pi '^\s*\d+\s+\d+\s+ACCEPT\s+udp\s+\*\s+\*\s+::\/0\s+::\/0\s+state\s+established')
      l_input_icmp_established=$(ip6tables -L -v -n | grep -Pi '^\s*\d+\s+\d+\s+ACCEPT\s+icmp\s+\*\s+\*\s+::\/0\s+::\/0\s+state\s+established')
      
      l_output_tcp_established=$(ip6tables -L -v -n | grep -Pi '^\s*\d+\s+\d+\s+ACCEPT\s+tcp\s+\*\s+\*\s+::\/0\s+::\/0\s+state\s+new,established')
      l_output_udp_established=$(ip6tables -L -v -n | grep -Pi '^\s*\d+\s+\d+\s+ACCEPT\s+udp\s+\*\s+\*\s+::\/0\s+::\/0\s+state\s+new,established')
      l_output_icmp_established=$(ip6tables -L -v -n | grep -Pi '^\s*\d+\s+\d+\s+ACCEPT\s+icmp\s+\*\s+\*\s+::\/0\s+::\/0\s+state\s+new,established')
      ...
      # if ruleset passes, we pass
		if [ "$l_input_ruleset" = passed ] && [ "$l_output_ruleset" = passed ] ; then
			...
			echo "- End check - Ensure ip6tables outbound and established connections are configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_ip6tables_outbound_established_connections_configured_fix()
	{
		echo -e "- Start remediation - Ensure ip6tables outbound and established connections are configured" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- These example rules can be used if desired:\n ip6tables -A OUTPUT -p tcp -m state --state NEW,ESTABLISHED -j ACCEPT\n ip6tables -A OUTPUT -p udp -m state --state NEW,ESTABLISHED -j ACCEPT\n ip6tables -A OUTPUT -p icmp -m state --state NEW,ESTABLISHED -j ACCEPT\n ip6tables -A INPUT -p tcp -m state --state ESTABLISHED -j ACCEPT\n ip6tables -A INPUT -p udp -m state --state ESTABLISHED -j ACCEPT\n ip6tables -A INPUT -p icmp -m state --state ESTABLISHED -j ACCEPT" | tee -a "$LOG" 2>> "$ELOG"
      test=manual
      
		echo -e "- End remediation - Ensure ip6tables outbound and established connections are configured" | tee -a "$LOG" 2>> "$ELOG"
	}
