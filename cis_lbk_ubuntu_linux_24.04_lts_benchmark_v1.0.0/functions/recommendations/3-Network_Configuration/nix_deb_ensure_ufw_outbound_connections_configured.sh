...
#
# # START METADATA
#   recommendation = 09293aa5
#   function = deb_ensure_ufw_outbound_connections_configured
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ufw_outbound_connections_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown        01/6/23    Recommendation "Ensure ufw outbound connections are configured"
#

deb_ensure_ufw_outbound_connections_configured()
{
	...
	deb_ensure_ufw_outbound_connections_configured_chk()
	{
		echo "- Start check - Ensure ufw outbound connections are configured" | tee -a "$LOG" 2>> "$ELOG"
		...
		if grep -Piq -- '^\h*Anywhere\h+ALLOW\h+OUT\h+Anywhere\h+on\h+all' <<< "$l_ufw_ruleset"; then
			l_output="$l_output\n- ufw rule 'Anywhere ALLOW OUT Anywhere on all' found"
		else
			l_output2="$l_output2\n- ufw rule 'Anywhere ALLOW OUT Anywhere on all' NOT found"
		fi

		#Check for ipv6 rules
		if [ "$l_ipv6" != "disabled" ]; then
			if grep -Piq -- '^\h*Anywhere\h+\(v6\)\h+ALLOW\h+OUT\h+Anywhere\h+\(v6\)\h+on\h+all' <<< "$l_ufw_ruleset"; then
				l_output="$l_output\n- ufw rule 'Anywhere (v6) ALLOW OUT Anywhere (v6) on all' found"
			else
				l_output2="$l_output2\n- ufw rule 'Anywhere (v6) ALLOW OUT Anywhere (v6) on all' NOT found"
			fi
		fi
		...
		if [ -z "$l_output2" ] ; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure ufw outbound connections are configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			...
			echo -e "- End check - Ensure ufw outbound connections are configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_ufw_outbound_connections_configured_fix()
	{
		echo -e "- Start remediation - Ensure ufw outbound connections are configured" | tee -a "$LOG" 2>> "$ELOG"
		
       	echo -e "- ufw rule 'ufw allow out on all' should be added" | tee -a "$LOG" 2>> "$ELOG"
		...
		echo -e "- End remediation - Ensure ufw outbound connections are configured" | tee -a "$LOG" 2>> "$ELOG"
	}
