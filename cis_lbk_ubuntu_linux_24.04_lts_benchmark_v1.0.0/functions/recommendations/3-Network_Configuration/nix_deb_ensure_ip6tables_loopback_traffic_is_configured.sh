...
#
# # START METADATA
#   recommendation = 90058faa
#   function = deb_ensure_ip6tables_loopback_traffic_is_configured
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ip6tables_loopback_traffic_is_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       06/28/22    Recommendation "Ensure ip6tables loopback traffic is configured"
#

deb_ensure_ip6tables_loopback_traffic_is_configured()
{
   ...
   ipv6_chk()
   {
	  ...
	  && sysctl net.ipv6.conf.default.disable_ipv6 | grep -Pq -- "^\h*net\.ipv6\.conf\.default\.disable_ipv6\h*=\h*1\h*(#.*)?$" \
	  && l_passing="true"
      
      if [ "$l_passing" = true ] ; then
         echo -e "IPv6 is disabled on the system" | tee -a "$LOG" 2>> "$ELOG"
         export G_IPV6=false
      else
         echo -e "IPv6 is enabled on the system" | tee -a "$LOG" 2>> "$ELOG"
         export G_IPV6=true
      fi
   }
	
	deb_ensure_ip6tables_loopback_traffic_is_configured_chk()
	{
		echo "- Start check - Ensure ip6tables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
		output="" l_in_lo_rule="" l_in_127_rule="" l_out_lo_rule="" l_ruleset=""
		            
		# Get INPUT Chain rules
      	l_in_lo_rule="$(ip6tables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+lo\s+\*')"
      	l_in_127_rule="$(ip6tables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+DROP\s+all\s+\*\s+\*\s+::1')"
      
      	# Get OUTPUT Chain rules
      	l_out_lo_rule="$(ip6tables -L OUTPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+\*\s+lo')"
      
		if [ -n "$l_in_lo_rule" ]; then
			echo "- Found INPUT rule for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
			if [ -n "$l_in_127_rule" ]; then
				echo "- Found INPUT rule for loopback network" | tee -a "$LOG" 2>> "$ELOG"
				if [ -n "$l_out_lo_rule" ]; then
				echo "- Found OUTPUT rule for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
				l_ruleset=passed
				else
				echo "- NO OUTPUT rule for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
				fi
			else
		...
		if [ "$l_ruleset" = passed ] ; then
			echo "- The ip6tables rules for loopback are configured" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure ip6tables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo "- The ip6tables rules for loopback are NOT configured" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure ip6tables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_ip6tables_loopback_traffic_is_configured_fix()
	{
		echo -e "- Start remediation - Ensure ip6tables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"

      if [ -z "$(ip6tables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+lo\s+\*')" ]; then
         echo "- Adding ip6tables INPUT rules for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
         ip6tables -A INPUT -i lo -j ACCEPT && test=remediated
      fi
      
      if [ -z "$(ip6tables -L INPUT -v -n | grep -P '^\s*\d+\s+\d+\s+DROP\s+all\s+\*\s+\*\s+::1')" ]; then
         echo "- Adding ip6tables INPUT rules for loopback network" | tee -a "$LOG" 2>> "$ELOG"
         ip6tables -A INPUT -s ::1 -j DROP && test=remediated
      fi
      
      if [ -z "$(ip6tables -L OUTPUT -v -n | grep -P '^\s*\d+\s+\d+\s+ACCEPT\s+all\s+\*\s+lo')" ]; then
         echo "- Adding ip6tables OUTPUT rules for loopback interface" | tee -a "$LOG" 2>> "$ELOG"
         ip6tables -A OUTPUT -o lo -j ACCEPT && test=remediated
      fi
      
		echo -e "- End remediation - Ensure ip6tables loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
	}
	...
	echo "- IPv6 status is: $G_IPV6" | tee -a "$LOG" 2>> "$ELOG"
	if [ "$G_FWIN" = "UKn" ]; then
