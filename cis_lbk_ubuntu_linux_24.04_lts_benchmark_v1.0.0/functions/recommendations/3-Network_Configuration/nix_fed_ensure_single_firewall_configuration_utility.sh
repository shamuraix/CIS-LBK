...
#
# # START METADATA
#   recommendation = cf620b21
#   function = fed_ensure_single_firewall_configuration_utility
#   applicable =
# # END METADATA
#
#
...
# ------------------------------------------------------------------------------------------------
# David Neilson	     04/24/23	 Recommendation "Ensure a single firewall configuration utility is in use"
# David Neilson      05/06/23    Added "tee -a" to every "echo -e" line
# Randie Bejar       09/20/23    Added manual remediation required after nftables is installed
...
fed_ensure_single_firewall_configuration_utility()
{
	...
	{
        echo "- Start check - Ensure a single firewall configuration utility is in use" | tee -a "$LOG" 2>> "$ELOG"
        ...
        if [ -z "$l_output2" ]; then
			...
			echo -e "- End check - Ensure a single firewall configuration utility is in use" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
        else
            echo -e "\n- Audit Results:\n ** Fail **\n$l_output2\n" | tee -a "$LOG" 2>> "$ELOG"
            echo -e "- FAILED:\n- Ensure single firewall configuration utility in use" | tee -a "$LOG" 2>> "$ELOG"
		...
		case $l_fwutil_status in
            ...
            enabled:*:enabled:*)
                ...
                if [ "$(awk -F: '{print $2}' <<< "$l_fwutil_status")" = "active" ] && [ "$(awk -F: '{print $4}' <<< "$l_fwutil_status")" = "inactive" ]; then
                    echo " - masking NFTables utility" | tee -a "$LOG" 2>> "$ELOG"
                    systemctl stop nftables && systemctl --now mask nftables
                elif [ "$(awk -F: '{print $4}' <<< "$l_fwutil_status")" = "active" ] && [ "$(awk -F: '{print $2}' <<< "$l_fwutil_status")" = "inactive" ]; then
                    echo " - masking FirewallD utility" | tee -a "$LOG" 2>> "$ELOG"
            ...
            *:active:*:active)
                ...
                if [ "$(awk -F: '{print $1}' <<< "$l_fwutil_status")" = "enabled" ] && [ "$(awk -F: '{print $3}' <<< "$l_fwutil_status")" != "enabled" ]; then
                    echo " - stopping and masking NFTables utility" | tee -a "$LOG" 2>> "$ELOG"
                    systemctl stop nftables && systemctl --now mask nftables
                elif [ "$(awk -F: '{print $3}' <<< "$l_fwutil_status")" = "enabled" ] && [ "$(awk -F: '{print $1}' <<< "$l_fwutil_status")" != "enabled" ]; then
                    echo " - stopping and masking FirewallD utility" | tee -a "$LOG" 2>> "$ELOG"
                    systemctl stop firewalld && systemctl --now mask firewalld
                fi ;;
            :enabled:active)
                echo -e "\n - NFTables utility is in use, enabled, and active\n - FirewallD package is not installed\n - no remediation required" | tee -a "$LOG" 2>> "$ELOG" ;;
            :)
                echo -e "\n - Neither FirewallD or NFTables is installed." | tee -a "$LOG" 2>> "$ELOG"
                echo "- Please refer to the remediation guidance and follow the appropriate section of the benchmark \"Configure iptables\", \"Configure firewalld\" or \"Configure nftables\"" | tee -a "$LOG" 2>> "$ELOG"
                l_test="manual" ;;
            *:*:)
                echo -e "\n - NFTables package is not installed on the system" | tee -a "$LOG" 2>> "$ELOG"
                echo "- Please refer to the remediation guidance and follow the appropriate section of the benchmark \"Configure iptables\", \"Configure firewalld\" or \"Configure nftables\"" | tee -a "$LOG" 2>> "$ELOG"
                l_test="manual" ;;
            *)
