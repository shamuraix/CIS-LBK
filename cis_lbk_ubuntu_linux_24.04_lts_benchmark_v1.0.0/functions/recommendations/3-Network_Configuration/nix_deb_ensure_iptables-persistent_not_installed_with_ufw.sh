...
#
# # START METADATA
#   recommendation = 5ae30056
#   function = deb_ensure_iptables-persistent_not_installed_with_ufw
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_iptables-persistent_not_installed_with_ufw.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       05/18/22    Recommendation "Ensure iptables-persistent is not installed with ufw"
#

deb_ensure_iptables-persistent_not_installed_with_ufw()
{
	...
	deb_ensure_iptables-persistent_not_installed_with_ufw_chk()
	{
		echo "- Start check - Ensure iptables-persistent is not installed with ufw" | tee -a "$LOG" 2>> "$ELOG"
		output=""
		...
		$G_PQ iptables-persistent >/dev/null && output="- The iptables-persistent package IS installed on the system"
		
		# If package doesn't exist, we pass
		if [ -z "$output" ] ; then
			echo "- The iptables-persistent package is NOT installed" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure iptables-persistent is not installed with ufw" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_iptables-persistent_not_installed_with_ufw_fix()
	{
		echo -e "- Start remediation - Ensure iptables-persistent is not installed with ufw" | tee -a "$LOG" 2>> "$ELOG"
		$G_PR iptables-persistent
		echo -e "- End remediation - Ensure iptables-persistent is not installed with ufw" | tee -a "$LOG" 2>> "$ELOG"
	}
