...
#
# # START METADATA
#   recommendation = 5445e53a
#   function = deb_ensure_nftables_rules_permanent
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_nftables_rules_permanent.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       06/30/22    Recommendation "Ensure nftables rules are permanent"
#

deb_ensure_nftables_rules_permanent()
{
	...
	deb_ensure_nftables_rules_permanent_chk()
	{
		echo "- Start check - Ensure nftables rules are permanent" | tee -a "$LOG" 2>> "$ELOG"
		nft_config="" l_input_rule="" l_forward_rule="" l_output_rule="" l_rule_tst=""
		...
		# Test for include
		if grep -Eqs "^\s*include" $nft_config; then
            # Test for rules
			l_input_rule="$(awk '/hook input/,/}/' $(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' $nft_config))"
			l_forward_rule="$(awk '/hook forward/,/}/' $(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' $nft_config))"
			l_output_rule="$(awk '/hook output/,/}/' $(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' $nft_config))"

			if [ -z "$l_input_rule" ]; then
				echo -e "- NO INPUT rules found in include file." | tee -a "$LOG" 2>> "$ELOG"
			else
				echo -e "- INPUT rules found in include file:\n$l_input_rule\n" | tee -a "$LOG" 2>> "$ELOG"
			fi
				
			if [ -z "$l_forward_rule" ]; then
				echo -e "- NO FORWARD rules found in include file." | tee -a "$LOG" 2>> "$ELOG"
			else
				echo -e "- FORWARD rules found in include file:\n$l_forward_rule\n" | tee -a "$LOG" 2>> "$ELOG"
			fi
				
			if [ -z "$l_output_rule" ]; then
				echo -e "- NO OUTPUT rules found in include file." | tee -a "$LOG" 2>> "$ELOG"
			else
				echo -e "- OUTPUT rules found in include file:\n$l_output_rule\n" | tee -a "$LOG" 2>> "$ELOG"
			fi
			
			# Set test result
			if [ -n "$l_input_rule" ] && [ -n "$l_forward_rule" ] && [ -n "$l_output_rule" ]; then
				l_rule_tst=passed
			fi
		else
			echo -e "- NO include line found in $nft_config" | tee -a "$LOG" 2>> "$ELOG"
		fi
		
		if [ "$l_rule_tst" = passed ]; then
			echo "- The nftables rules are included in an include file" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure nftables rules are permanent" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo "- The nftables rules are NOT included in an include file" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure nftables rules are permanent" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_nftables_rules_permanent_fix()
	{
		echo -e "- Start remediation - Ensure nftables rules are permanent" | tee -a "$LOG" 2>> "$ELOG"
		
      echo -e "- Edit the $nft_config file and un-comment or add a line with 'include \"<Absolute path to nftables rules file>\"' for each nftables file you want included in the nftables ruleset on boot." | tee -a "$LOG" 2>> "$ELOG"
      echo -e "- Example: include \"/etc/nftables/nftables.rules\"" | tee -a "$LOG" 2>> "$ELOG"
      test=manual
      
		echo -e "- End remediation - Ensure nftables rules are permanent" | tee -a "$LOG" 2>> "$ELOG"
	}
