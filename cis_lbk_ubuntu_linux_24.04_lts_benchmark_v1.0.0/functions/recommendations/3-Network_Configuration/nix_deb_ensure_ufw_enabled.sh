...
#
# # START METADATA
#   recommendation = 5f1734de
#   function = deb_ensure_ufw_enabled
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ufw_enabled.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       05/18/22    Recommendation "Ensure ufw service is enabled"
#

deb_ensure_ufw_enabled()
{
	...
	deb_ensure_ufw_enabled_chk()
	{
		echo "- Start check - Ensure ufw service is enabled" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""
		...
		if systemctl is-active ufw | grep -Pq -- '^\s*active'; then
		...
		if ufw status | grep -Pq -- '^\s*Status:\s+active'; then
            l_output="$l_output\n- UFW is active"
        else
            l_output2="$l_output2\n- UFW is NOT active"
        fi
		...
		if [ -z "$l_output2" ] ; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure ufw service is enabled" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			...
			echo -e "- End check - Ensure ufw service is enabled" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_ufw_enabled_fix()
	{
		echo -e "- Start remediation - Ensure ufw service is enabled" | tee -a "$LOG" 2>> "$ELOG"
		
        # Enable ufw.service
		if ! systemctl is-enabled ufw | grep -q 'enabled'; then
            if systemctl is-enabled ufw | grep -Pq -- '^\s*masked'; then
                echo -e "- Unmasking ufw service" | tee -a "$LOG" 2>> "$ELOG"
                systemctl unmask ufw
                echo -e "- Enabling and starting ufw service" | tee -a "$LOG" 2>> "$ELOG"
        ...
        # Check if UFW is active
		if ! ufw status | grep -Pq -- '^\s*Status:\s+active'; then
            echo -e "- UFW is not enabled. Enabling UFW without a verified configuration could result in immediate disconnection of user sessions." | tee -a "$LOG" 2>> "$ELOG"
            echo -e "- Enable UFW with the following command:\n  ufw --force enable" | tee -a "$LOG" 2>> "$ELOG"
            echo -e "- This item should be remediated manually." | tee -a "$LOG" 2>> "$ELOG"
            l_test="manual"
        fi

		echo -e "- End remediation - Ensure ufw service is enabled" | tee -a "$LOG" 2>> "$ELOG"
	}
