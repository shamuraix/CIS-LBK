...
#
# # START METADATA
#   recommendation = 8b70fd84
#   function = deb_ensure_ufw_installed
#   applicable =
# # END METADATA
#
...
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       05/18/22    Recommendation "Ensure ufw is installed"
#
...
deb_ensure_ufw_installed()
{
	...
	deb_ensure_ufw_installed_chk()
	{
		echo "- Start check - Ensure ufw is installed" | tee -a "$LOG" 2>> "$ELOG"
		output=""
		...
		! $G_PQ ufw >/dev/null && output="- The ufw package is not installed on the system"

		# If package doesn't exist, we pass
		if [ -z "$output" ] ; then
			echo "- The ufw package is installed" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure ufw is installed" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_ufw_installed_fix()
	{
		echo -e "- Start remediation - Ensure ufw is installed" | tee -a "$LOG" 2>> "$ELOG"
		$G_PM install -y ufw
		echo -e "- End remediation - Ensure ufw is installed" | tee -a "$LOG" 2>> "$ELOG"
	}
