...
#
# # START METADATA
#   recommendation = 16615375
#   function = deb_ensure_nftables_base_chains_exist
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_nftables_base_chains_exist.sh
# 
...
deb_ensure_nftables_base_chains_exist()
{
	...
	deb_ensure_nftables_base_chains_exist_chk()
	{
		echo "- Start check - Ensure nftables base chains exist" | tee -a "$LOG" 2>> "$ELOG"
      ...
      # if ruleset passes, we pass
		if [ "$l_ruleset" = passed ] ; then
			...
			echo "- End check - Ensure nftables base chains exist" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	deb_ensure_nftables_base_chains_exist_fix()
	{
		echo -e "- Start remediation - Ensure nftables base chains exist" | tee -a "$LOG" 2>> "$ELOG"
		...
		echo -e "- End remediation - Ensure nftables base chains exist" | tee -a "$LOG" 2>> "$ELOG"
	}
