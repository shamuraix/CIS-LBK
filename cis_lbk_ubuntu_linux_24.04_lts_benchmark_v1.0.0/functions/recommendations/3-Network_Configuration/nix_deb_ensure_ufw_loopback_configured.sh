...
#
# # START METADATA
#   recommendation = 2e1f9583
#   function = deb_ensure_ufw_loopback_configured
#   applicable =
# # END METADATA
#
...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_ufw_loopback_configured.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown        01/6/23    Recommendation "Ensure ufw loopback traffic is configured"
#

deb_ensure_ufw_loopback_configured()
{
	...
	deb_ensure_ufw_loopback_configured_chk()
	{
		echo "- Start check - Ensure ufw loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
		...
		if grep -Piq -- '^\h*Anywhere\h+on\h+lo\h+ALLOW\h+IN\h+Anywhere' <<< "$l_ufw_ruleset"; then
			l_output="$l_output\n- ufw rule 'Anywhere on lo ALLOW IN Anywhere' found"
		else
			l_output2="$l_output2\n- ufw rule 'Anywhere on lo ALLOW IN Anywhere' NOT found"
		fi

		if grep -Piq -- '^\h*Anywhere\h+ALLOW\h+OUT\h+Anywhere\h+on\h+lo' <<< "$l_ufw_ruleset"; then
			l_output="$l_output\n- ufw rule 'Anywhere ALLOW OUT Anywhere on lo' found"
		else
			l_output2="$l_output2\n- ufw rule 'Anywhere ALLOW OUT Anywhere on lo' NOT found"
		fi

		if grep -Piq -- '^\h*Anywhere\h+DENY\h+IN\h+127\.0\.0\.0\/8' <<< "$l_ufw_ruleset"; then
			l_output="$l_output\n- ufw rule 'Anywhere DENY IN 127.0.0.0/8' found"
		else
			l_output2="$l_output2\n- ufw rule 'Anywhere DENY IN 127.0.0.0/8' NOT found"
		fi		

		#Check for ipv6 rules
		if [ "$l_ipv6" != "disabled" ]; then
			if grep -Piq -- '^\h*Anywhere\h+\(v6\)\h+on\h+lo\h+ALLOW\h+IN\h+Anywhere\h+\(v6\)' <<< "$l_ufw_ruleset"; then
				l_output="$l_output\n- ufw rule 'Anywhere (v6) on lo ALLOW IN Anywhere (v6)' found"
			else
				l_output2="$l_output2\n- ufw rule 'Anywhere (v6) on lo ALLOW IN Anywhere (v6)' NOT found"
			fi

			if grep -Piq -- '^\h*Anywhere\h+\(v6\)\h+ALLOW\h+OUT\h+Anywhere\h+\(v6\)\h+on\h+lo' <<< "$l_ufw_ruleset"; then
				l_output="$l_output\n- ufw rule 'Anywhere (v6) ALLOW OUT Anywhere (v6) on lo' found"
			else
				l_output2="$l_output2\n- ufw rule 'Anywhere (v6) ALLOW OUT Anywhere (v6) on lo' NOT found"
			fi

			if grep -Piq -- '^\h*Anywhere\h+\(v6\)\h+DENY\h+IN\h+::1' <<< "$l_ufw_ruleset"; then
				l_output="$l_output\n- ufw rule 'Anywhere (v6) DENY IN ::1' found"
			else
				l_output2="$l_output2\n- ufw rule 'Anywhere (v6) DENY IN ::1' NOT found"
			fi
		fi
		...
		if [ -z "$l_output2" ] ; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure ufw loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			...
			echo -e "- End check - Ensure ufw loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	deb_ensure_ufw_loopback_configured_fix()
	{
		echo -e "- Start remediation - Ensure ufw loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
		
        if ! grep -Piq -- '^\h*Anywhere\h+on\h+lo\h+ALLOW\h+IN\h+Anywhere' <<< "$l_ufw_ruleset" || (! grep -Piq -- '^\h*Anywhere\h+on\h+lo\h+ALLOW\h+IN\h+Anywhere' <<< "$l_ufw_ruleset" && [ "$l_ipv6" != "disabled" ]); then
			echo -e "- ufw rule 'ufw allow in on lo' should be added" | tee -a "$LOG" 2>> "$ELOG"
		fi

		if ! grep -Piq -- '^\h*Anywhere\h+ALLOW\h+OUT\h+Anywhere\h+on\h+lo' <<< "$l_ufw_ruleset" || (! grep -Piq -- '^\h*Anywhere\h+on\h+lo\h+ALLOW\h+IN\h+Anywhere' <<< "$l_ufw_ruleset" && [ "$l_ipv6" != "disabled" ]); then
			echo -e "- ufw rule 'ufw allow out on lo' should be added" | tee -a "$LOG" 2>> "$ELOG"
		fi

		if ! grep -Piq -- '^\h*Anywhere\h+DENY\h+IN\h+127\.0\.0\.0\/8' <<< "$l_ufw_ruleset"; then
			echo -e "- ufw rule 'ufw deny in from 127.0.0.0/8' should be added" | tee -a "$LOG" 2>> "$ELOG"
		fi

		if ! grep -Piq -- '^\h*Anywhere\h+\(v6\)\h+DENY\h+IN\h+::1' <<< "$l_ufw_ruleset" && [ "$l_ipv6" != "disabled" ]; then
			echo -e "- ufw rule 'ufw deny in from ::1' should be added" | tee -a "$LOG" 2>> "$ELOG"
		fi

		echo -e "- UFW loopback traffic rules are NOT in place. Enabling UFW rules without a verified configuration could result in immediate disconnection of user sessions." | tee -a "$LOG" 2>> "$ELOG"
		 echo -e "- This item should be remediated manually." | tee -a "$LOG" 2>> "$ELOG"
		l_test="manual"
        
		echo -e "- End remediation - Ensure ufw loopback traffic is configured" | tee -a "$LOG" 2>> "$ELOG"
	}
