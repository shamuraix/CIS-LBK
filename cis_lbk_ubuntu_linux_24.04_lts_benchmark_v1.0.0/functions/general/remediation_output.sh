...
remediation_output()
{
	case "$output_code" in
		...
		104)
			...
			echo "*****************************************************************" | tee -a "$SLOG" 2>> "$ELOG"
			# Send N/A recommendations to the SKIPLOG
			echo "*****************************************************************" | tee -a "$SKIPLOG" 2>> "$ELOG"
			echo " - $RN - $RNA -" | tee -a "$SKIPLOG" 2>> "$ELOG"
			echo " - N/A - Recommendation is non applicable" | tee -a "$SKIPLOG" 2>> "$ELOG"
			echo "*****************************************************************" | tee -a "$SKIPLOG" 2>> "$ELOG"
			;;
		105)
			...
			echo "*****************************************************************" | tee -a "$SLOG" 2>> "$ELOG"
			# Send excluded recommendations to the SKIPLOG
			echo "*****************************************************************" | tee -a "$SKIPLOG" 2>> "$ELOG"
			echo " - $RN - $RNA -" | tee -a "$SKIPLOG" 2>> "$ELOG"
			echo " - EXCLUDED - Recommendation on the excluded list" | tee -a "$SKIPLOG" 2>> "$ELOG"
			echo "*****************************************************************" | tee -a "$SKIPLOG" 2>> "$ELOG"
			;;
		106)
